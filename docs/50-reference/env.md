...
└── docs/
    └── 50-reference/env.md  ← Este arquivo
```
...
## Apêndice — Transformação inicial (resumo do SETUP_VARIAVEIS_ENV.md)

Documento original: `SETUP_VARIAVEIS_ENV.md` (mantido como stub). Principais pontos:
- Remoção de senhas do código e criação do template `.env.example`.
- Criação de `src/config.py` para carregar/validar variáveis.
- Scripts de carregamento: `load_env.ps1` e `load_env.sh`.
- Orientações de atualização para scripts Python que ainda usavam credenciais hardcoded.
