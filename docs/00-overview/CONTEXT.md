...
> 📚 **ÍNDICE CENTRALIZADO:** Consulte [`docs/00-overview/README.md`](./README.md) (novo índice) para navegação completa de todos os documentos, referências e métricas.
...
- ✅ **Implantação em produção (PHASE 1):** executar `docs/20-operations/checklists/PRODUCTION_DEPLOYMENT_CHECKLIST.md` e validar com `phase1_execute.ps1`.
- 🔍 **Validação pós-deploy:** coletar resultados em `artifacts/results/*_results.json` e validar métricas (CDC latency, RLAC overhead, BI latency).
...
- 🛠️ **Execução Phase 1 (Quick):** use `docs/20-operations/checklists/PHASE_1_CHECKLIST.md` ou `etc/scripts/phase1_checklist.ps1` para rodar a validação rápida e coletar resultados em `artifacts/results/`.
...
👉 **[`docs/50-reference/env.md`](../50-reference/env.md)** - Guia completo com exemplos para todos os shells
...
docs/40-troubleshooting/PROBLEMAS_ESOLUCOES.md (Problemas & Soluções)
```
